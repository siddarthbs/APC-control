...
class SpiralZipper:
	...
	def __init__(sz, start_position, r_winch, c, looptime, armflag):
		...
		sz.rb = 0.225           # circle containing spiral zipper  #.22 for the APC arm, .11 for the RCTA arm
		sz.rt = 0.0695  	       # circle containing the spiral zipper column
		sz.r_winch = r_winch   # the radius of the winch used to wind tether.

		phi = 28.6 * pi/180 #pi/6  #rad
		...
		sz.L = [0,0,0,0] #tether length initialization routine
