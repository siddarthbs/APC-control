...
class SpiralZipper:
	...
	def __init__(sz, start_position, r_winch, c, looptime):
		...
		for port in range(3):
			if got_port[port]:
				...
				while (not figuredout_LIDAR and not figuredout_GYRO):
		...
		p2 = sz.rb*np.array([np.cos(phi),-np.sin(phi), 0]) + np.array([0,0,.04])
		p3 = sz.rb*np.array([-np.cos(phi),-np.sin(phi), 0]) + np.array([0,0,.04])
	...
	def update_state (sz, c):
		...
		if sz.tether_subtract_CCW:
			...
			sz.L_vel_actual[0] = (- dtheta[0] * 0.00955) / looptime #0.06/(2*3.14159))/sz.looptime #gets the current speed of the system
			#sz.L_vel_actual[1] = (sz.L[1] - (sz.calc_L1([sz.L[2], sz.L[3]]))/sz.looptime
			sz.L_vel_actual[2] =  (-dtheta[1] * sz.r_winch) / looptime
			sz.L_vel_actual[3] =  (-dtheta[2] * sz.r_winch) / looptime
		...
		print "current tether lengths are : "
		print sz.L
		print "current actual tether velocities are : "
		print sz.L_vel_actual
		sz.timeold = now()
	...
	def update_goal(sz,delta): #gives a new 
	...
	def set_tether_speeds(sz):
		...
		print L_goal
		position = sz.sensed_pos
		#L_actual = sz.cart2tether(position,False)

		for i in range(4):
			looptime = now() - sz.timeold2  #normally would just use sz.looptime. Attempts to compensate somewhat for variances in loopspeed caused by motor sensing problems
			sz.L_vel_desired[i] = (L_goal[i] - sz.L[i]) / looptime

		print "desired tether velocities are : "
		print sz.L_vel_desired 
		sz.timeold2 = now()
	...
	def get_sensor_readings(sz):
		...
		while (len(l) != 3) and (len(l2) != 3):
			...
			if (len(l2) != 3 and len(l) != 3):
				sleep(0.05)
		#print "Sensor A Readings:"
		#print l
		print "Sensor B Readings:"
		print l2

		return l2 #check which sensor is hooked up.  Change between l and l2 as necessary


	def get_lidar_readings(sz):  #reads lidar sensor data back
		l = []
		while (len(l) != 1):
			bytesToRead = sz.ser_lidar.inWaiting()
			readings = sz.ser_lidar.read(bytesToRead)
			l = []
			startread = False
			for t in reversed(readings.split()):  # read from most recent serial data
				if (t is '>'):
...
#		p = [float(x) for x in p]
	...
	def PI_control(sz): #velocity PI control
		...
		command_torques = [t2,t3,t1]
		command_torques = [t1,t2,t3]

		for i in range(3):
			if command_torques[i] > 0.7:
				command_torques[i] =  0.7
			if command_torques[i] < -0.7:
				command_torques[i] = -0.7

		return command_torques

	def actuate_Motors(sz, c):  #converts desired user input commands into useable servo command inputs
